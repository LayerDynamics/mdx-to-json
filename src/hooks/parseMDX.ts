...
const handleMDXJsNodes: Plugin = () => {
  return (tree) => {
    visit(tree, (node: any) => {
      if (node.type === 'mdxjsEsm' || node.type === 'mdxFlowExpression') {
        // Convert mdxjsEsm and mdxFlowExpression nodes to HTML comments
        node.type = 'html';
        node.value = `<!-- ${node.type}: ${node.value} -->`;
      } else if (node.type === 'mdxJsxFlowElement' || node.type === 'mdxJsxTextElement') {
        // Convert MDX JSX elements to HTML comments
        node.type = 'html';
        node.value = `<!-- MDX JSX Element: ${node.name} -->`;
      }
    });
  };
};
// Hook to parse MDX content with enhanced error handling
// Updated useParseMDX hook
export const useParseMDX = () => {
  ...
  const parseMDX = useCallback((content: string) => {
    ...
    try {
      ...
      console.log("Tree processed:", processedTree);

      const file = processor.stringify(processedTree);
...
// Optional: Export other utility functions if needed
export {
  markdownSyntaxHandler,
  jsCommentHandler,
  htmlPreProcessor,
  checkMDXSyntax,
  fallbackParse,
