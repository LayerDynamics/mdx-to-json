...
// Enhanced HTML PreProcessor to handle unexpected slashes
const htmlPreProcessor = (content: string): string => {
    try {
        ...
        processedContent = processedContent.replace(/<\/([a-zA-Z]+)\/>/g, (match, tag) => {
            console.warn(`Unexpected closing slash in tag detected: </${tag}/> - fixing...`);
            return `</${tag}>`;
        });

        processedContent = processedContent.replace(/<([a-zA-Z]+)(?:\s[^>]*)?>/g, (match, tag) => {
    ...
    return (tree: any) => {
        ...
        safeVisit(tree, (node, index, parent) => {
            if (
                ...
                (isNodeType(node, 'mdxjsEsm') or isNodeType(node, 'mdxJsxFlowElement'))
            ) {
  ...
  const parseMDX = useCallback((content: string) => {
    ...
    try {
      ...
      const processor = unified()
        ...
        .use(rehypeSanitize)
        .use(rehypeStringify);  // Add this line to ensure the compiler is included
      ...
      const file = processor.stringify(tree);  // Correctly use stringify with the compiler
