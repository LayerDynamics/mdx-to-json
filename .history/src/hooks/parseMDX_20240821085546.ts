...
const safeVisit = (tree: any, callback: (node: any, index: number, parent: any) => void) => {
    visit(tree, (node, index, parent) => {
        // Ensure node is defined and is an object before proceeding
        if (!node || typeof node !== 'object') {
            console.warn('Encountered invalid node:', node);
            return; // Skip this node if it's not valid
        }
        
        // Check if 'children' property exists before accessing it
        if (parent && !Array.isArray(parent.children)) {
            console.warn('Parent node has no valid children array:', parent);
            return; // Skip processing if parent has invalid children
        }
...
// Function to visit nodes with additional safety checks
const visit2 = (node: any) => {
    // Ensure node is defined before accessing properties
    if (node && 'children' in node) {
        ...
        console.warn('Node is undefined or does not have children property:', node);
    }
};
...
export const useNodeMapper = () => {
    const nodeMapper = (tree: any) => {
        safeVisit(tree, (node) => {
            mapNodes(node);
            visit2(node); // Ensure visit2 is called after mapping nodes
        });
    };
