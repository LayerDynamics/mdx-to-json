...
const handleSpecialComments = () => {
    return (tree: any) => {
        if (!tree) {
            console.warn("Invalid tree structure: undefined in handleSpecial Comments");
            return;
        }
...
// Custom handler to explicitly manage MDXJS nodes
const handleMDXJsNodes = () => {
  return (tree) => {
    const removeMDXJsEsmNodes = (node, index, parent) => {
      if (node.type === 'mdxjsEsm') {
        console.log(`Removing 'mdxjsEsm' node:`, node);
        if (parent) {
          parent.children.splice(index, 1); // Remove the node entirely from the parent's children array
          return index - 1; // Adjust current index since we've removed an item
        }
      }
      return index; // Return current index if no node is removed
    };

    safeVisit(tree, (node, index, parent) => {
      return removeMDXJsEsmNodes(node, index, parent);
    });
  };
};
...
const parseMDX = useCallback((content: string) => {
  ...
  try {
    ...
    const processor = unified()
      ...
      .use(handleMDXJsNodes)  // Make sure this handles `mdxjsEsm` before anything else
      .use(rehypeSanitize)
      .use(nodeMapper)  // Process nodes with any remaining transformations
      .use(rehypeStringify);
