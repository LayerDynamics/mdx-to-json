...
const htmlPreProcessor = (content: string): string => {
    try {
        // Fix tags with an unexpected closing slash
        let processedContent = content.replace(/<\/\s*([^>\s]+)\s*\/>/g, (match, tagName) => {
            console.warn(`Unexpected closing slash in tag detected: </${tagName}/> - fixing...`);
            return `</${tagName}>`;
        });

        // Fix self-closing tags that are incorrectly formatted
        processedContent = processedContent.replace(/<([^>\s]+)\s*\/>/g, (match, tagName) => {
            console.warn(`Self-closing tag corrected: <${tagName} />`);
            return `<${tagName}></${tagName}>`;
        });

        // Replace any unclosed tags that might be incorrectly formatted
        ...
        // Other simplifications and sanitizations as needed
  ...
  const parseMDX = useCallback((content: string) => {
    ...
    try {
      ...
      const processor = unified()
        ...
        .use(rehypeSanitize)
        .use(rehypeStringify);  // Adding a compiler to stringify the tree back to an HTML string
