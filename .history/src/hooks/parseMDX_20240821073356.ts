...
// Utility Function: Log the structure of a node
const logNodeStructure = (node: any, depth: number = 0): void => {
    const prefix = ' '.repeat(depth * 2);
    console.log(`${prefix}Node Type: ${node.type}`);
    if (hasChildren(node)) {
        node.children.forEach((child: any) => logNodeStructure(child, depth + 1));
    }
};
...
const checkMDXSyntax = (content: string): boolean => {
    try {
        // Basic checks for unclosed tags and unmatched braces
