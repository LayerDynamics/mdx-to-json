...
const htmlPreProcessor = (content: string): string => {
    try {
        ...
        // Example: Replace specific problematic tags
        ...
        // Example: Remove inline scripts (these can cause issues in MDX)
        processedContent = processedContent.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '<!-- Script removed -->');

        // Further simplifications or sanitizations can be added here
        // For example, removing style tags or converting them to inline styles
    ...
    safeVisit(tree, (node, index, parent) => {
        if (isNodeType(node, 'html')) {
            try {
                ...
                // Example transformation for video tags or other special handling
    ...
    const parseMDX = useCallback((content: string) => {
        ...
        try {
            ...
            const processor = unified()
                ...
                .use(rehypeSanitize)
                .use(nodeMapper)  // Now properly using nodeMapper from useNodeMapper
                .use(rehypeStringify);
            ...
            if (error instanceof Error) {
                setError(new Error(`MDX parsing error: ${error.message}. Content: ${content}`));
            } else {
                setError(new Error('Unknown MDX parsing error occurred.'));
            }
