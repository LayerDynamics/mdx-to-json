...
const handleMDXJsNodes = () => {
  return (tree) => {
    safeVisit(tree, (node, index, parent) => {
      // Check if the node is an `mdxjsEsm` type and remove it
      if (node.type === 'mdxjsEsm') {
        console.log(`Removing 'mdxjsEsm' node:`, node);
        // Remove the node entirely from the parent's children array
        if (parent && Array.isArray(parent.children)) {
          parent.children.splice(index, 1);
          return index - 1; // Adjust the index after removal
        }
      }
      return index; // Return the current index if no node is removed
    });
  };
};
  ...
  const parseMDX = useCallback((content: string) => {
    ...
    try {
      ...
      const processor = unified()
        ...
        .use(handleMDXJsNodes) // Make sure this handles mdxjsEsm nodes as early as possible
        .use(rehypeSanitize)
