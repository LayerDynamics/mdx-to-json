...
const handleMDXJsNodes = () => {
    return (tree: any) => {
        safeVisit(tree, (node, index, parent) => {
            // Transform or remove 'mdxjsEsm' nodes
            if (node.type === 'mdxjsEsm') {
                console.log(`Transforming 'mdxjsEsm' node: ${node.value}`);
                // Transform node to a comment or remove it
                node.type = 'comment';
                node.value = `<!-- Omitted ESM Import: ${node.value} -->`;
            }
        });
    };
};
